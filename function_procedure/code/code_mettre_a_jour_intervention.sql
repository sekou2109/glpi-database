...
begin
   if p_nouveau_statut not in ( 'En_attente',
                                'En_cours',
                                'Resolu' ) then
      raise_application_error(
         -20001,
         'Statut invalide. Utiliser : En_attente, En_cours, Resolu'
      );
   end if;
   ...
   update intervention
      set statut = p_nouveau_statut,
          date_debut =
             case
                when p_nouveau_statut = 'En_cours' then
                   sysdate
                else
                   date_debut
             end,
          date_fin =
             case
                when p_nouveau_statut = 'Resolu' then
                   sysdate
                else
                   null
             end
