...
CREATE MATERIALIZED VIEW log_technicien_cergy ON technicien_cergy;
...
CREATE MATERIALIZED VIEW log_technicien_pau ON technicien_pau;

CREATE MATERIALIZED VIEW technicien_pau REFRESH ON COMMIT FAST SELECT * FROM technicien WHERE id_site=(SELECT id_site FROM site WHERE nom='Pau');
